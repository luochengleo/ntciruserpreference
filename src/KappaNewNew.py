...
def direction(n):
    ...
    if n<0:
        return -1
fout = open('../data/kapparesultnewnew.csv','w')
lr = open('../data/leftright.csv','w')
def process(i):
    if i<=1 and i>=-1:
        return 0
    else:
        return i
for f in listdir('../data/pairnew'):
    left,right = f.strip().replace('.csv','').split('vs')
    lines = open('../data/pairnew/'+f).readlines()
    ...
    s = float(leftbeats+tie+rightbeats)
    lr.write(','.join([left,right,str(float(leftbeats)/float(s)),str(float(tie)/float(s)),str(float(rightbeats)/float(s))])+'\n')
    fout.write('\n')
lr.close()
